...
**핵심 안전 원리**: claude 활성 세션도 입력 대기 중엔 CPU 0% 입니다. 그래서 "지금 CPU 낮음"만으로 죽이면 활성 세션을 죽입니다. v1.4 는 CLI 본체와 Bash/WSL 같은 자손의 **활동 이력을 시간에 걸쳐 누적**하고, 종료 직전에도 다시 확인해서 `idleMinutes` **연속 무활동**인 세션만 정리합니다.
