...
| 상황 | 판정 |
|---|---|
| claude helper(`bg-spare`/`bg-pty-host`) 인데 **PPID=1**, `claude daemon` 도 죽어있음 | **고아** |
| claude helper 인데 PPID=1, **daemon 은 살아있음** | **정상** — daemon 이 detach 스폰한 것 (v1.5.0) |
...
> **v1.5.0 변경**: daemon 이 살아있는 동안에도 활성 세션을 호스팅하는 pty-host 의 PPID 가
> 1 인 것이 실측으로 확인돼 (2026-07 오탐 사고), helper 의 고아 판정에 "daemon 생존 확인"
> 조건을 추가했습니다. 또한 idle 스윕(`--idle-only`) 경로에서는 시작 `minAgeMinutes`(기본
> 30분) 미만 프로세스가 종료 후보가 되지 않습니다 (수동 `--orphans-only`/기본 모드는 미적용).
...
v1.5.0 에서 이 판정이 대폭 손질됐습니다:

- **CPU 율은 단일 코어 기준**입니다. 이전엔 전체 코어 수로 나눠서 (10코어 기준 5분당
  CPU 15초 필요) API 대기 위주 세션이 영원히 "무활동"으로 잡혔습니다. 이제 임계
  `cpuThresholdPct 0.5` = 5분당 CPU 1.5초입니다. Claude 도 Codex 처럼 자손 트리 CPU 를 합산합니다.
- **커맨드라인 변경 시 이력 리셋** — daemon 이 미리 띄운 `bg-spare` 가 세션 호스트로 전환되면
  argv 가 바뀌는데, 이전엔 몇 시간짜리 유휴 이력을 그대로 상속받아 "방금 연 세션"이 죽었습니다.
- **잠자기 시간은 유휴가 아님** — 관측 공백(잠자기/트래커 중단)이 추적 간격의 3배를 넘으면
  그만큼 유휴 시계를 앞으로 보정합니다. 부팅 직후엔 `idleMinutes` 동안 스윕 자체를 건너뜁니다.
- **메모리 여유 게이트** — 가용 RAM 이 `minFreePct`(기본 25%) 이상이면 정리를 통째로 생략합니다.
- **보호 목록** — `tracker-settings.json` 의 `protected` 배열(커맨드라인 부분 일치)에 걸리는
  프로세스는 절대 자동 종료하지 않습니다 (상시 리스너·서비스용).
