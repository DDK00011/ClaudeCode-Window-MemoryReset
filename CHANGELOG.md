...
- **P0: `-KeepPids` command injection** — value flows through `Start-Process -Verb RunAs -ArgumentList` to re-elevate. Pre-v1.3.0-final, no input validation existed: `-KeepPids '1234"; Start-Process calc.exe; #'` could execute arbitrary code in the elevated PowerShell. Fix: regex whitelist `^[\d, \t]*$` enforced **before** any UAC elevation, at the very top of the script (line ~73). Reject + exit 1 on any character outside digits/comma/space/tab. (An earlier draft used `^[\d,\s]*$` — `\s` matches `\n` / `\r` / `\f` / `\v`, which can confuse PowerShell command-line parsing during the UAC argv roundtrip. Narrowed to explicit `[\d, \t]`.)
